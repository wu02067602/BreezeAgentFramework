...
class LLMConnector:
    """使用 OpenAI 官方 SDK 的 LLM 連線器（支援 OpenAI-compatible 端點）。

    本類別以簡潔設計為原則，透過 SDK 的 `chat.completions.create` 實作
    三種常見互動：
    - 單次對答（僅 user 提示）
    ...
    例外情況：
        ...
        Exception: SDK 呼叫或服務端錯誤。

    相依與相容性說明：
        - 以 SDK 方式呼叫 Chat Completions；可將 `api_base_url` 指向相容端點。
    ...
    def __init__(
        ...
        if not api_base_url:
            raise ValueError("api_base_url 不可為空。")
        if not default_model:
            raise ValueError("default_model 不可為空。")
        if timeout <= 0:
    ...
    def chat_with_history(
        ...
        *,
        ...
        resp = self._client.chat.completions.create(**{k: v for k, v in params.items() if v is not None})
        msg = resp.choices[0].message
        return msg.content or ""
