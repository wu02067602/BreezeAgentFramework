...
class PromptManager:
    ...
    def build_planning_prompt(self, question: str, tool_schemas: List[Any]) -> str:
        ...
        prompt = f"""
        你是AI助理，請分析使用者的問題，對於需要創意發想、建議或規劃的問題（例如旅行計畫、推薦、規劃等），如果你有足夠的內部知識來提供一個合理的回答，請優先直接用自然語言回答。當你需要獲取特定事實資料來支援回答時，才考慮使用工具。
        ...
        如果只需要調用單一工具，請直接回傳該工具的 JSON 呼叫格式，例如：
            {{"tool_name": "工具名", "parameters": {{參數}}}}
